...
Description:
    ...
    Mass Copy Actions To other Tracks

Menus:

    Flame 2025+:

        Timeline -> Copy Actions -> Copy To
    ...
    def copy_actions(self):
        ...
        if not os.path.exists(self.action_path):
            try:
                ...
                action_path = self.action_path + '/copy_to.action_node'
            except:
                action_path = '/var/tmp/copy_to.action_node'
                    ...
                    if segment in self.selection:
                        ...
                        if len(source_segments.segments) == len(target_segments.segments):
                            for tlfx in segment.effects:
                                if tlfx.type == 'Action':
                                    # print ("Source Segment: ", segment.name)
                                    ...
                                    # print ("Target Segment Name: ", target_segment.name)
                                    # print ("Segment Count: ", segment_count)
                                    try:
                                        # Try to Create and Load Setup
                                        action_fx = target_segment.create_effect('Action')
                                        action_fx.load_setup(action_path)
                                    except:
                                        # If that fails, just try to load the setup
                                        for tlfx in target_segment.effects:
                                            if tlfx.type == 'Action':
                                                tlfx.load_setup(action_path)
                                    # target_segment.colour = (50,50,50)
                        else:
                            self.window.close()
