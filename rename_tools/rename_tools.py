...
- Improved rename execution consistency and undo handling.
...
def _macos_yes_no(title, question, default_yes=True):
    ...
    try:
        ...
        script = (
            ...
            f"with title {_osascript_string(_dialog_title(title))} "
...
def _prompt_text_with_idem(kind, title, label, default_text, checkbox_label="Prevent duplicate addition (idempotent)"):
    ...
    if sys.platform.startswith("linux") and _which("zenity"):
        try:
            ...
            p2 = subprocess.run([
                "zenity", "--question", "--title", _dialog_title(title),
                "--text", checkbox_label
            ], capture_output=True, text=True)
            if p2.returncode == 0:
                idem_val = True
...
def _prompt_find_replace(title="Find & Replace", label_find="Find:", label_repl="Replace:"):
    ...
    if sys.platform.startswith("linux") and _which("zenity"):
        try:
            p1 = subprocess.run(["zenity", "--entry", "--title", _dialog_title(title), "--text", label_find, "--entry-text", _load_last_find()], capture_output=True, text=True)
            if p1.returncode != 0:
                return None, None
            p2 = subprocess.run(["zenity", "--entry", "--title", _dialog_title(title), "--text", label_repl, "--entry-text", _load_last_repl()], capture_output=True, text=True)
            if p2.returncode != 0:
                return None, None
...
def show_rename_panel(selection=None):
    ...
    parent = _parent_widget()
    dlg = QtWidgets.QDialog(parent)
    dlg.setWindowTitle(_dialog_title(SCRIPT_DISPLAY_NAME))
    dlg.setModal(False)
